...
!...  get log lengths, dibs
     ...
     &            LOGLEN, LOGDIA, NOLOGP, NOLOGS, TLOGS,COEFFS) 
      ...
      IF (DEBUG%MODEL) THEN
         DO 550 I=1,TLOGS
         WRITE  (LUDBG, 530)'LOGDIA ', I, LOGDIA(I,1),'LOGLEN ', I,
     &     LOGLEN(I)
...
!...  get cubic log volumes'
      CALL R9LOGCUFT(TLOGS, LOGLEN, LOGDIA, LOGVOL, TLOGVOL, tcfVol)     
  ...
  650    CONTINUE
         ...
         WRITE  (LUDBG, 675)'total cubic ', tcfvol, 'total log ',TLOGVOL
  675    FORMAT (A, F7.2, A, F7.2)
      END IF
...
!...
      StTot=stemHt/TOTHT
      IF(LOG(1.-StTot).LT.(-20./R))StTot=1.
